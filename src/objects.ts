...
export function isCorrect(question: Question, answer: string): boolean {
    return answer.toLowerCase().trim() == question.expected.toLowerCase();
}
...
export function toShortForm(question: Question): string {
    const trimmedQName: string = question.name.slice(0, 10);
    return `${question.id}: ${trimmedQName}`;
}
...
export function toMarkdown(question: Question): string {
    ...
    let formattedOptions = "# " + name + "\n" + body;
    // let formattedOptions = `
    //     # ${name}
    //     ${body} \n`;
    if (type === "multiple_choice_question") {
        options.map((option) => (formattedOptions += "\n- " + option));
    }
